...
if not os.path.exists("data/processed/discovery_insights.json") or not os.path.exists("data/processed/opportunity_report.csv") or not os.path.exists("data/processed/analyzed_reviews.csv"):
    try:
...
def load_insights_data():
    if os.path.exists("data/processed/discovery_insights.json"):
        with open("data/processed/discovery_insights.json", "r", encoding="utf-8") as f:
            return json.load(f)
    return None

@st.cache_data
def load_opportunity_report():
    if os.path.exists("data/processed/opportunity_report.csv"):
        return pd.read_csv("data/processed/opportunity_report.csv")
    return None

@st.cache_data
def load_analyzed_reviews():
    if os.path.exists("data/processed/analyzed_reviews.csv"):
        return pd.read_csv("data/processed/analyzed_reviews.csv")
    return None
    ...
    if st.sidebar.button("Reset Database", use_container_width=True):
        with st.spinner("Resetting to baseline seed data..."):
            try:
                ...
                for filepath in ["data/raw/google_play/play_store_reviews.json", "data/raw/app_store/app_store_reviews.json", "data/raw/reddit/reddit_comments.json"]:
...
summary_path = "data/reports/source_summary.csv"
if not os.path.exists(summary_path):
    summary_path = "data/source_summary.csv"
...
if os.path.exists(summary_path):
    ...
    clean_path = "data/clean_master_dataset.csv"
...
# ----------------- Top KPI Section -----------------
clean_df_path = "data/clean_master_dataset.csv"
